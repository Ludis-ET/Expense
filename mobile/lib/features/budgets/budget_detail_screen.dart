...
class _BudgetDetailScreenState extends State<BudgetDetailScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final json = await context
          .read<ApiClient>()
          .get<Map<String, dynamic>>('/budgets/${widget.budgetId}');
      if (!mounted) return;
      setState(() {
  ...
  Widget _body(BuildContext context, BudgetDetail detail, PrefsState prefs) {
    ...
    return ListView(
      ...
      children: [
        FadeInUp(child: _Hero(detail: detail, tint: tint, money: money, onAction: _heroAction)),
        ...
        if (b.isClosed) ...[
          ...
          FadeInUp(
            ...
            child: _Banner(
              ...
              text: 'This plan is closed — it no longer appears when you add a '
        ...
        if (!b.isUnplanned && !b.started) ...[
          ...
          FadeInUp(
            ...
            child: _Banner(
              ...
              text: 'This plan starts on ${formatDate(b.startsAt)}. You can fill it now, '
        ...
        FadeInUp(
          ...
          child: b.isUnplanned ? _UnplannedStats(detail: detail, money: money) : _FundedStats(b: b, detail: detail, money: money),
        ),
    ...
    switch (action) {
      ...
      case 'close':
        ...
        final ok = await confirm(
          context,
          title: 'Close ${b.name}?',
          message: 'Anything still in the pot goes back to the wallets that '
      ...
      case 'delete':
        ...
        final ok = await confirm(
          context,
          title: 'Delete ${b.name}?',
          message: 'Expenses already recorded against it stay as ordinary expenses. '
      ...
      child: Column(
        ...
        children: [
                    ...
                    Wrap(
                      ...
                      children: [
                        ...
                        if (b.kind == BudgetKind.recurring)
                          AppBadge('resets ${b.recurrenceLabel ?? 'periodically'}',
                              tone: BadgeTone.info, dense: true)
                        else
          ...
          Text(
            ...
            style: TextStyle(fontSize: 11.5, height: 1.45, color: t.mutedForeground),
          ),
          if (adjusted != 0) ...[
...
class _FundedStats extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        Expanded(child: _StatTile(label: 'Planned', value: money(b.plannedAmount))),
...
class _UnplannedStats extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      children: [
        Expanded(child: _StatTile(label: 'Spent unplanned', value: money(b.spentAmount))),
        const SizedBox(width: 8),
        Expanded(
          child: _StatTile(
            label: 'All time',
      ...
      child: Column(
        ...
        children: [
          CardTitleRow(
            ...
            hint: 'This money is still physically in these accounts — it just '
    ...
    return AppCard(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Expanded(child: _StatTile(label: 'Filled', value: money(detail.lifetimeAllocated))),
              const SizedBox(width: 8),
              Expanded(child: _StatTile(label: 'Spent', value: money(detail.lifetimeSpent))),
            ],
          ),
          const SizedBox(height: 8),
          Row(
