...
**Budget** (the user calls these *plans*) — `name`, `kind` (ONE_TIME | RECURRING |
...
**BudgetAdjustment** — a signed change to what a plan is *meant* to hold, with
...
1. **A point of view.** What is this page *for*? State the two or three questions it
...
3. **Every metric, derived precisely.** For each number or chart: the exact
   ...
   window, how transfers and UNPLANNED are handled), and — more importantly — *what
   a person does differently* after reading it. Kill any metric that fails that
   second test.
4. **At least three things a generic budgeting app could not show**, built from the
   ...
   against the figure a cycle *opened* with, carry-over behaviour across cycles,
