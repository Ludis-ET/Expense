...
import { Sparkles, Plus, Trash2, ExternalLink, Star } from "lucide-react";
import {
  PageHeader,
  Skeleton,
  EmptyState,
...
export default function WishlistPage() {
  ...
  async function setStatus(item: WishlistItem, status: WishlistStatus) {
    try {
      await api.put(`/wishlist/${item.id}`, { status });
      toast.success(status === "BOUGHT" ? "Marked bought!" : "Updated");
      void mutate();
    } catch (err) {
      toast.error(err instanceof ApiError ? err.message : "Failed");
    }
      ...
      {data?.stats && (
        <div className="relative overflow-hidden rounded-2xl border border-border bg-gradient-to-br from-amber-50 via-rose-50 to-violet-100 p-5 dark:from-amber-950/40 dark:via-rose-950/30 dark:to-violet-950/40 sm:p-6">
          ...
          <div className="mt-3 grid grid-cols-2 gap-4 sm:grid-cols-4">
            ...
            <div>
              <p className="text-2xl font-bold">{data.stats.bought}</p>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 flex items-center gap-1 text-xs text-muted">
                    ...
                    {item.goal && <span>· linked to {item.goal.name}</span>}
                  </p>
                </div>
              </div>
