 ...
 */
export async function outlookHistory(
  user: AuthUser,
  currency?: string,
  months = 6,
) {
  ...
  const patternStart = new Date(
    now.getTime() - PATTERN_WINDOW_DAYS * 86_400_000,
  );
  ...
  const monthCells: MonthCell[] = [...buckets.entries()].map(
    ([month, cell]) => ({
      month,
  ...
  const repeatCandidates = [...groups.values()]
    ...
    .map((g) => {
      ...
      const avgGapDays =
        count > 1 ? Math.max(1, spanDays / (count - 1)) : DAYS_PER_MONTH;
