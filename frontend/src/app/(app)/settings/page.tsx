...
import { AvatarPicker, BannerPicker } from '@/components/settings/profile-look-pickers';
...
const SECTIONS: { id: string; label: string; short: string; icon: LucideIcon }[] = [
        ...
        <div className="flex flex-col gap-3 px-4 pb-4 sm:flex-row sm:items-end sm:justify-between sm:gap-4 sm:px-7 sm:pb-6">
          <div className="-mt-8 flex min-w-0 items-end gap-3 sm:-mt-12 sm:gap-4">
            ...
            <div className="min-w-0 pb-0.5 sm:pb-1">
              <h1 className="truncate text-base font-bold tracking-tight sm:text-2xl">{user?.name}</h1>
              <p className="truncate text-xs text-muted sm:text-sm">{user?.email}</p>
            </div>
          </div>
              ...
              {SECTIONS.map((s) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="max-w-full truncate leading-none sm:hidden">{s.short}</span>
                    <span className="hidden max-w-full truncate leading-none sm:inline">{s.label}</span>
                  </button>
                );
              })}
            </div>
              ...
              <CardContent className="pt-5 sm:pt-6">
                <form onSubmit={save} className="space-y-5">
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted">Pick an illustrated look for your profile.</p>
                    <AvatarPicker
                      value={form.avatarId}
                      onChange={(avatarId) => setForm((f) => ({ ...f, avatarId }))}
                    />
                  </div>
                  <div className="space-y-2">
                    <p className="text-sm font-medium">Settings banner</p>
                    <p className="text-xs text-muted">Choose the header art at the top of this page.</p>
                    <BannerPicker
                      value={form.bannerId}
                      onChange={(bannerId) => setForm((f) => ({ ...f, bannerId }))}
                    />
                  </div>
