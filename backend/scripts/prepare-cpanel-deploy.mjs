...
for (const rel of COPY_PATHS) {
  ...
  if (!existsSync(src)) {
    if (rel === "generated") {
      log(
        "COPY",
        `SKIP   ${rel} missing (server postinstall will generate Prisma client)`,
      );
      continue;
    }
    if (rel === "package-lock.json") {
      log(
        "COPY",
        `WARN   ${rel} missing; cPanel npm install may resolve versions differently`,
      );
      continue;
    }
  ...
  if (st.isDirectory()) {
    ...
    log(
      "COPY",
      `DIR  ${rel}/ → ${files} files (${formatBytes(dirSize(dest))})`,
    );
  } else {
    cpSync(src, dest);
...
for (const name of readdirSync(outDir).sort()) {
  ...
  if (st.isDirectory()) {
    log(
      "SUMMARY",
      `  📁 ${name}/ (${walkFiles(full).length} files, ${formatBytes(dirSize(full))})`,
    );
  } else {
