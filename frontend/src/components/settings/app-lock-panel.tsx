    ...
    <Card>
      <CardHeader>
        <div>
          ...
          <CardDescription>
            PIN and biometrics (Face ID, Touch ID, Windows Hello, fingerprint) keep Santim private on this
            device. Secrets never leave your browser.
          </CardDescription>
        </div>
      </CardHeader>
        ...
        {!lock.enabled ? (
          <div className="rounded-xl border border-dashed border-border bg-surface-muted/40 p-5 text-center">
            ...
            <p className="mt-1 text-xs text-muted">
              Anyone with your open session can see balances until you enable a PIN.
            </p>
            <Button className="mt-4" size="sm" onClick={() => setSetupOpen(true)}>
              Set up PIN lock
            </Button>
          </div>
        ) : (
          <>
            ...
            <div className="space-y-3">
              <Field label="Auto-lock">
                <Select
                  ...
                  onChange={(e) => lock.setAutoLockMinutes(Number(e.target.value) as AutoLockMinutes)}
                >
              ...
              <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-border px-4 py-3">
                ...
                <span>
                  ...
                  <span className="mt-0.5 block text-xs text-muted">
                    Re-locks after you hide the tab or leave the PWA for a short while.
                  </span>
                </span>
              </label>
            </div>
            ...
            <div className="rounded-xl border border-border p-4">
              <div className="flex items-start justify-between gap-3">
                ...
                {lock.hasBiometric ? (
                  <Button size="sm" variant="outline" onClick={() => lock.removeBiometrics()}>
                    Remove
                  </Button>
                ) : (
                  <Button
      ...
      <DisableLockModal open={disableOpen} onClose={() => setDisableOpen(false)} />
    </Card>
  );
}

function SetupPinModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Set up app lock" description="Choose a 4–8 digit PIN for this device.">
      <form onSubmit={submit} className="space-y-4">
        ...
        {biometricAvailable && (
          <label className={cn('flex items-start gap-2 text-sm')}>
            <input type="checkbox" className="mt-1" checked={enrollBio} onChange={(e) => setEnrollBio(e.target.checked)} />
            <span>
              <span className="font-medium">Also enroll biometrics</span>
              <span className="block text-xs text-muted">Face ID, Touch ID, Windows Hello, or fingerprint.</span>
            </span>
          </label>
        )}
...
function DisableLockModal({ open, onClose }: { open: boolean; onClose: () => void }) {
